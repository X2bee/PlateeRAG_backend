...
def _prompt_pdf_single_md(html_reference: str) -> str:
    return (
        ...
        f"{html_reference}\n\n"
        "**🔥 복잡한 표 구조 처리 규칙 (최우선):**\n"
        "1. **병합된 셀 완전 분석**: 세로로 병합된 셀은 해당하는 모든 행에 동일한 내용을 복사해서 넣어주세요\n"
        "   - 예: '구분' 셀이 3행에 걸쳐 병합되어 있다면, 3개 행 모두에 '구분' 입력\n"
        "2. **계층적 구조 인식**: 대분류-중분류-소분류가 있는 경우 각각을 별도 행으로 만들어주세요\n"
        "   - 예: '신용장방식' 아래 '수출환어음 추심', '수출환어음 매입' 등이 있으면 각각 행으로 분리\n"
        "3. **빈 셀 의미적 처리**: 위 행의 내용이 계속되는 경우 명시적으로 내용을 복사해주세요\n"
        "4. **다중 정보 셀 분해**: 한 셀에 여러 정보가 있으면 주요 정보를 우선하되, 상세 정보는 괄호나 부연설명으로 포함\n"
        "5. **금액/조건 정확 추출**: '$100이하', 'BBB-' 같은 조건은 정확히 별도 열에 배치\n\n"
        "**일반 변환 규칙:**\n"
        "6. HTML 참고로 누락/불분명 텍스트 보완\n"
        "7. 완전한 마크다운 문법 사용(#, 표, 목록, 강조)\n"
        "8. 논리적 섹션으로 구분\n"
        "9. 한국어/영어만 사용, 한자는 한국어로\n\n"
        "**표 변환 상세 지침:**\n"
        "- 모든 행과 열을 빠짐없이 변환\n"
        "- 병합된 셀의 내용은 해당하는 모든 셀에 동일하게 입력\n"
        "- 계층 구조는 들여쓰기나 '-' 기호로 표현하지 말고, 각각을 독립된 행으로 만들기\n"
        "- 조건, 금액, 권한 등은 명확히 구분되는 열에 배치\n"
        "- 긴 설명이나 비고사항은 '비고' 열을 활용\n\n"
        "**표 설명 작성:**\n"
        "10. 각 표마다 **주석**으로 다음 정보 포함:\n"
        "    - 표의 목적과 내용 요약\n"
        "    - 주요 컬럼별 의미 설명\n"
        "    - 특이사항이나 조건 설명\n"
        "    - 데이터 해석 시 주의사항\n\n"
        ...
        "이렇게 변환:\n"
        "```markdown\n"
        "| 구분 | 업무내용 | 신용등급 | 금액조건 | 권한 |\n"
        "|------|----------|----------|----------|------|\n"
        "| 신용 | 신용대출 | BBB- | $100이하 | 전결 |\n"
        "| 신용 | 신용대출 | BB+ | $50이하 | 전결 |\n"
        "| 담보 | 4,5호 담보 | - | - | 전결 |\n"
        "\n"
        "**주석) 표 설명**: 이 표는 대출 유형별 신용등급과 금액 조건에 따른 결재 권한을 보여줍니다. 신용대출의 경우 BBB- 등급은 100달러 이하, BB+ 등급은 50달러 이하까지 전결 처리 가능하며, 담보대출(4,5호)의 경우 금액 제한 없이 전결 가능합니다.\n"
        "```"
    )
...
def _prompt_merged_pages_md(html_reference: str, num_pages: int) -> str:
    return (
        f"이 이미지는 {num_pages}개의 PDF 페이지를 세로로 합친 것입니다. "
        "각 페이지를 구분하여 정확한 마크다운으로 변환해주세요.\n\n"
        "**HTML 참고 텍스트:**\n"
        f"{html_reference}\n\n"
        "**🔥 복잡한 표 구조 처리 규칙 (최우선):**\n"
        "1. **페이지별 표 연속성 확인**: 표가 여러 페이지에 걸쳐 있으면 하나의 완전한 표로 합치기\n"
        "2. **병합된 셀 완전 복원**: 세로 병합된 셀은 모든 해당 행에 동일 내용 입력\n"
        "3. **계층적 분류 체계 인식**: 대분류-중분류-소분류를 각각 독립된 행으로 분리\n"
        "4. **조건/금액 정보 정확 분리**: 한 셀에 있는 여러 조건을 적절한 열로 분산\n"
        "5. **빈 셀 의미적 보완**: 상위 행의 내용이 계속되는 경우 명시적으로 복사\n\n"
        "**페이지 구분 및 변환 규칙:**\n"
        "6. **페이지 구분**: 각 페이지의 경계를 인식하고 `## 페이지 N` 헤더로 구분\n"
        "7. **HTML 참고 활용**: 위의 참고 텍스트를 활용하여 불분명한 텍스트 보완\n"
        "8. **표 구조 완전 복원**: 병합된 셀을 포함하여 정확한 마크다운 표로 변환\n"
        "9. **페이지 연속성**: 표나 내용이 페이지를 걸쳐 이어지는 경우 적절히 연결\n"
        "10. **정확한 텍스트**: 모든 문자, 숫자, 기호를 정확히 인식\n"
        "11. **구조 보존**: 제목, 부제목, 목록, 단락 구분을 명확히 표현\n"
        "12. **특수 형식 유지**: 날짜, 금액, 주소 등의 형식을 정확히 보존\n"
        "13. **한국어/영어만**: 한자는 한국어로 변환\n\n"
        "**표 변환 상세 지침:**\n"
        "- 복잡한 병합 구조도 모든 셀을 채워서 완전한 직사각형 표로 만들기\n"
        "- 계층적 항목들은 들여쓰기 대신 별도 행으로 분리\n"
        "- 조건, 등급, 금액, 권한 등을 명확한 열로 구분\n"
        "- 긴 설명이나 조건문은 '비고' 열에 배치\n\n"
        "**표 설명 작성:**\n"
        "14. **각 표마다 상세한 주석 추가**:\n"
        "    - 표의 목적과 전체 내용 요약\n"
        "    - 각 컬럼의 의미와 데이터 유형 설명\n"
