...
def _prompt_merged_pages_md(html_reference: str, num_pages: int) -> str:
    return (
        ...
        f"총 {num_pages}개 페이지를 구분하여 정확한 마크다운으로 변환해주세요.\n\n"
        
        "**중요 지침:**\n"
        "- 반드시 한국어로만 출력하세요. 중국어, 영어 등 다른 언어 사용 금지\n"
        "- HTML 참고 텍스트의 표 구조를 정확히 따라주세요\n"
        "- 이미지에서 보이는 실제 표 레이아웃을 유지하세요\n"
        "- 임의로 내용을 추가하거나 변경하지 마세요\n\n"
        
        "**HTML 참고 텍스트:**\n"
        f"{html_reference}\n\n"
        
        "**변환 규칙 (우선순위 순):**\n"
        "1. 이미지에서 보이는 실제 표 구조를 최우선으로 따름\n"
        "2. HTML 참고 텍스트로 불분명한 텍스트만 보완 (구조 변경 금지)\n"
        "3. 표의 행/열 개수와 위치를 정확히 유지\n"
        "4. 병합된 셀은 해당하는 모든 셀에 동일한 내용 입력\n"
        "5. 빈 셀은 '-' 또는 공백으로 표시\n"
        "6. 페이지 연속성 고려 (표가 다음 페이지로 이어지는 경우)\n"
        "7. 표 외의 텍스트도 이미지에 보이는 그대로 유지\n\n"
        
        "**표 작성 시 주의사항:**\n"
        "- 표 헤더와 데이터의 정렬을 이미지와 동일하게 유지\n"
        "- 숫자, 날짜, 금액 등은 이미지에 표시된 형식 그대로 작성\n"
        "- 표 제목이나 캡션이 있다면 표 위/아래에 정확히 위치\n\n"
