...
async def get_or_create_execution_meta(
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            # DB에 저장
            insert_query = """
            INSERT INTO execution_meta (user_id, interaction_id, workflow_id, workflow_name, interaction_count, metadata, created_at)
            ...
            db_manager.config_db_manager.execute_query(
                insert_query,
                (user_id, interaction_id, workflow_id, workflow_name, 0, metadata_json)
            )
