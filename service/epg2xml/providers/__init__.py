...
class EPGProgram:
    ...
    def to_json(self, cfg: dict) -> dict:
            ...
            for cc in sorted(cast + crew, key=lambda x: TAG_CREDITS.index(x["title"])):
                ...
                for k, v in cc.items():
                    if k not in ["title", "name"]:
                        credit[k] = v
                program_json["credits"].append(credit)
...
class EPGProvider:
    ...
    def write_channels_json(self) -> List[dict]:
        """채널 정보를 JSON 형태로 반환"""
        channels = []
        for ch in self.req_channels:
            if not ch.programs:
    ...
    def write_programs_json(self) -> List[dict]:
...
class EPGHandler:
    ...
    def load_providers(self, cfgs: dict) -> List[EPGProvider]:
        ...
        for name, cfg in cfgs.items():
            if not cfg["ENABLED"]:
                continue
            try:
                m = import_module(f"service.epg2xml.providers.{name.lower()}")
    ...
    def print_json(self) -> None:
        """EPG 데이터를 JSON 형태로 콘솔에 출력"""
        import json
        epg_data = self.to_json()
        print(json.dumps(epg_data, ensure_ascii=False, indent=2))
