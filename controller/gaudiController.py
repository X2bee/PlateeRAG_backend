...
# ========== Helper Functions ==========
def get_gaudi_config(request: Request):
    """Gaudi 설정 가져오기"""
    config_composer = get_config_composer(request)
    return config_composer.get_config_by_category_name("gaudi")
                ...
                try:
                    ...
                    try:
                        memory_info = {
                            ...
                            "utilization": hpu.utilization(i) if hasattr(hpu, 'utilization') else 0
                        }
                    except:
                        memory_info = {"allocated": 0, "reserved": 0, "utilization": 0}
                    
                    device_info[i] = {
                        "name": name,
                        "memory": memory_info,
                        "allocated": i in hpu_allocation,
                        "instance_id": hpu_allocation.get(i, None)
                    }
                except Exception as e:
                    device_info[i] = {
                        "name": f"HPU-{i}: Error ({str(e)})",
                        "memory": {"allocated": 0, "reserved": 0, "utilization": 0},
                        "allocated": i in hpu_allocation,
                        "instance_id": hpu_allocation.get(i, None)
                    }
...
async def start_vllm_auto_allocation(
    request: Request, 
    vllm_config: GaudiVLLMConfigRequest,
    allocation_request: AutoAllocationRequest
):
    """VLLM 자동 할당 시작"""
    try:
        # HPU 가용성 확인
        ...
        # 텐서 병렬 크기와 요청된 HPU 개수 검증
        if vllm_config.tensor_parallel_size != allocation_request.required_hpus:
            logger.warning(f"텐서 병렬 크기({vllm_config.tensor_parallel_size})와 요청 HPU 개수({allocation_request.required_hpus})가 다릅니다. 텐서 병렬 크기로 조정합니다.")
            allocation_request.required_hpus = vllm_config.tensor_parallel_size
        ...
        if not available_hpus:
            ...
            raise HTTPException(
                status_code=400, 
                detail=f"요청된 {allocation_request.required_hpus}개 HPU를 할당할 수 없습니다. 사용 가능: {available_count}개"
            )
        
        # 포트 중복 확인 및 자동 할당
        if not check_port_available(vllm_config.port):
            new_port = get_next_available_port(vllm_config.port)
            if new_port is None:
                raise HTTPException(status_code=400, detail="사용 가능한 포트를 찾을 수 없습니다")
            logger.info(f"포트 {vllm_config.port}가 사용 중이므로 {new_port}로 변경합니다")
            vllm_config.port = new_port
...
@router.post("/vllm/start-manual",
    ...
    response_model=VLLMInstanceInfo)
async def start_vllm_manual_allocation(
    request: Request, 
    vllm_config: GaudiVLLMConfigRequest,
    allocation_request: ManualAllocationRequest
):
    """VLLM 수동 할당 시작"""
    try:
        # HPU 가용성 확인
        ...
        # 장치 ID 유효성 검사
        for device_id in allocation_request.device_ids:
        ...
        # 텐서 병렬 크기 검증
        if vllm_config.tensor_parallel_size != len(allocation_request.device_ids):
            logger.warning(f"텐서 병렬 크기({vllm_config.tensor_parallel_size})와 할당된 HPU 개수({len(allocation_request.device_ids)})가 다릅니다. HPU 개수로 조정합니다.")
            vllm_config.tensor_parallel_size = len(allocation_request.device_ids)
        
        # 포트 중복 확인
        if not check_port_available(vllm_config.port):
            raise HTTPException(
                status_code=400, 
                detail=f"포트 {vllm_config.port}가 이미 사용 중입니다"
            )
        
        return await _start_vllm_instance(vllm_config, allocation_request.device_ids)
