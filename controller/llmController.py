...
async def get_llm_status(request: Request):
    ...
    try:
        ...
        backend_log.info("Retrieved LLM status",
                        metadata={"current_provider": current_provider,
        ...
        backend_log.error("Failed to retrieve LLM status", exception=e)
...
async def switch_llm_provider(request: Request, request_context: dict):
    ...
    try:
        ...
        if not provider:
            backend_log.warn("Provider not specified in switch request")
            raise HTTPException(status_code=400, detail="Provider is required")

        if provider not in ["openai", "vllm", "sgl", "gemini", "anthropic"]:
            backend_log.warn(f"Invalid provider specified: {provider}")
        ...
        if default_provider_config:
            ...
            backend_log.success("Switched LLM provider",
                              metadata={"old_provider": old_value, "new_provider": provider})
            return {
                "status": "success",
            ...
            backend_log.error("DEFAULT_LLM_PROVIDER config not found")
            raise HTTPException(status_code=404, detail="DEFAULT_LLM_PROVIDER config not found")

    except Exception as e:
        backend_log.error("Error switching LLM provider", exception=e)
        raise HTTPException(status_code=500, detail="Internal Server Error")
...
async def test_connection(request: Request, category: str):
    ...
    try:
        config_composer = get_config_composer(request)
        llm_service = LLMService()
        backend_log.info(f"Starting connection test for category: {category}")
        ...
        if category == "openai":
            ...
            if current_provider == "openai":
                ...
                backend_log.warn(f"Unsupported LLM provider specified: {current_provider}")
                raise HTTPException(status_code=400, detail=f"Unsupported LLM provider: {current_provider}")

        else:
            backend_log.warn(f"Unsupported category specified for connection test: {category}")
        ...
        backend_log.error(f"Connection test failed for {category}: {e}")
...
async def test_collection_connection(request: Request, category: str):
    ...
    try:
        ...
        if const_provider not in ("openai", "vllm"):
            backend_log.warn(f"Unsupported collection provider specified: {const_provider}")
        ...
        if const_provider == "openai":
            ...
            backend_log.info(f"Testing OpenAI connection with config: {config_data}")
            return {
                    "status": "success",
            ...
            backend_log.info(f"Testing vLLM connection with config: {config_data}")
            return await llm_service.test_vllm_connection(config_data)

    except HTTPException:
        raise
    except (ValueError, ConnectionError, RequestException) as e:
        backend_log.error(f"Collection connection test failed for {category}: {e}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        backend_log.error(f"Collection connection test failed for {category}: {e}")
