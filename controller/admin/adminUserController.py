...
@router.post("/update-user/available-user-sections")
async def edit_user_available_user_sections(request: Request, user_data: dict):
    val_superuser = await validate_superuser(request)
    if val_superuser.get("superuser") is not True:
        raise HTTPException(
            status_code=403,
    ...
    try:
        ...
        db_user_info = db_user_info[0]
        available_sections = user_data.get("available_user_sections", db_user_info.available_user_sections)
        ...
        updates = {}
        if available_sections is not None:
            updates['available_user_sections'] = available_sections
