...
def process_hsmoa_response(raw_data):
    """
...
def process_hsmoa_v1_response(raw_data):
    """
...
def search_hsmoa_v1_api(query, source="tvshop", limit=10, direction="past"):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                # 응답 데이터 전처리
                processed_data = process_hsmoa_v1_response(raw_data)
...
def search_hsmoa_popular_api(query, source="uniq_tvshop", limit=10):
    """
...
def search_hsmoa_api(query, offset=0, limit=20, order="rel", site="", platform="web"):
    """
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                # 응답 데이터 전처리
                processed_data = process_hsmoa_response(raw_data)
...
if __name__ == "__main__":
    ...
    result_v2 = search_hsmoa_api(search_query)

    print("\n" + "="*50 + "\n")

    print("2. V1 API Test:")
    print(f"검색어: {search_query}")
    result_v1 = search_hsmoa_v1_api(search_query)

    print("\n" + "="*50 + "\n")

    print("3. Popular API Test:")
    print(f"검색어: {search_query}")
    result_popular = search_hsmoa_popular_api(search_query)
