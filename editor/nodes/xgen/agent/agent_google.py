...
from editor.nodes.xgen.agent.functions import rag_context_builder, create_json_output_prompt, create_tool_context_prompt, create_context_prompt
...
class AgentGoogleNode(Node):
    categoryId = "xgen"
    functionId = "agents"
    nodeId = "agents/google"
    nodeName = "Agent Google"
    description = "RAG 컨텍스트를 사용하여 채팅 응답을 생성하는 Google Gemini Agent 노드"
    tags = ["agent", "chat", "rag", "google", "gemini"]
    ...
    parameters = [
        {
            "id": "model", "name": "Model", "type": "STR", "value": "gemini-2.5-flash", "required": True,
            "options": [
    ...
    def execute(
        ...
        try:
            ...
            llm, tools_list, chat_history = prepare_llm_components_google(text, tools, memory, model, temperature, max_tokens, n_messages, streaming=False)
            ...
            logger.error(f"[AGENT_EXECUTE] Google Chat Agent 실행 중 오류 발생: {str(e)}")
