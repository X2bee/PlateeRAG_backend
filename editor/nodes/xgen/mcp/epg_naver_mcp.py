...
class EPGNaverMCP(Node):
    categoryId = "xgen"
    functionId = "mcp"
    nodeId = "mcp/epg_naver_mcp"
    nodeName = "EPG NAVER MCP"
    description = "NAVER EPG 데이터를 가져오는 MCP 노드입니다. 한국 홈쇼핑 채널들의 편성표 정보를 JSON 형태로 제공합니다."
    tags = ["mcp", "epg", "naver", "tv", "schedule"]
    ...
    def execute(self, use_cache: bool = True, target_date: str = "", **kwargs):
        def create_epg_naver_tool():
            ...
            @tool("epg_naver_mcp", description=description, args_schema=EPGChannelSchema)
            def epg_naver_tool(channel_name: str) -> str:
                """
                NAVER EPG에서 특정 채널의 편성표를 가져옵니다.
            ...
            return epg_naver_tool
        return create_epg_naver_tool()
